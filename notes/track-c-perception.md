...
- [x] 采购 650nm 一字线激光模组 USB（~15-50 元）
- [x] 激光线中心提取（灰度重心法 → 高斯拟合 → Steger）
- [x] 光平面标定（激光平面在相机坐标系的方程）
- [x] 三角测量计算：像素 → 相机坐标 → 实际 3D
- [x] 运动扫描 → 逐帧拼 3D 点云 → Open3D 可视化

**验证**：扫描手机/杯子，3D 轮廓可见，高度误差 < 1mm（30cm 近场）

**代码**：`code/perception/c2_laser_triangulation/`
